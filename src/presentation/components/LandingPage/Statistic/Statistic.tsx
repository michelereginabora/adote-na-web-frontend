...
const StatisticItem: React.FC<{ name: string; value: string }> = ({ name, value }) => {
  const { elementRef, isVisible } = useFadeInOnScroll<HTMLDivElement>();
  
  return (
    <div 
      ref={elementRef} 
      className={`mx-auto flex max-w-xs flex-col gap-y-4 transition-opacity duration-1000 ${isVisible ? 'opacity-100' : 'opacity-0'}`}
    >
    ...
    <div className="bg-[var(--color-bg-highlight)] py-16 sm:py-14">
      <div className="mx-auto max-w-7xl px-6 lg:px-8">
        ...
        <dl className="grid grid-cols-1 gap-x-8 gap-y-16 text-center lg:grid-cols-3">
          {stats.map(stat => (
            <StatisticItem key={stat.id} name={stat.name} value={stat.value} />
          ))}
        </dl>
      </div>
    </div>  
